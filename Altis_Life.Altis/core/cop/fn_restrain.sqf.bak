...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			titleText["Du wurdest automatisch befreit da du extrem lange gefesselt warst","PLAIN"];
		};
	};
};
		if(!(player getVariable["surrender",FALSE])) then { player setVariable["surrender",FALSE,TRUE]; };

titleText[format["Du wurdest gefesselt von %1",name _cop],"PLAIN"];
...
{
	titleText["Du wurdest befreit. Drücke 'V' um die Fesseln zu lösen","PLAIN"];
};